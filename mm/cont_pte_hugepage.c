...
void __cont_pte_set_huge_pte_at(struct mm_struct *mm, unsigned long addr,
			      pte_t *ptep, pte_t pte, bool flush)
{
...
void cont_pte_set_huge_pte_at(struct mm_struct *mm, unsigned long addr,
			pte_t *ptep, pte_t pte)
{
	__cont_pte_set_huge_pte_at(mm, addr, ptep, pte, true);
}
			      ...
			      pte_t *ptep)
{
	...
	pte = get_clear_flush(mm, addr, ptep, PAGE_SIZE, CONT_PTES, false);
...
static int proc_stat_show(struct seq_file *s, void *v)
{
	...
	for (i = 0; i < min_t(u64, cnt, (u64)16); i++) {
