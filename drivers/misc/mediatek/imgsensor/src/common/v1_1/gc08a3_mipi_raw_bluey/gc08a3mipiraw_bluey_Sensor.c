...
#include "gc08a3mipiraw_bluey_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A3_SENSOR_ID_BLUEY,
...
static kal_uint16 gc08a3_3264x1840_addr_data[] = {
/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
...
static kal_uint16 gc08a3_3264x2448_24fps_addr_data[] = {
/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
...
static void normal_video_setting(kal_uint16 currefps)
{
	...
	table_write_cmos_sensor(gc08a3_3264x1840_addr_data,
		sizeof(gc08a3_3264x1840_addr_data)/sizeof(kal_uint16));
...
{
	pr_debug("[%s] slim_video_Start, 3264x2448@30.19fps\n", __func__);
	table_write_cmos_sensor(gc08a3_3264x2448_addr_data,
		sizeof(gc08a3_3264x2448_addr_data)/sizeof(kal_uint16));
...
{
	pr_debug("[%s] custom1_Start, 3264x2448@24fps\n", __func__);
	table_write_cmos_sensor(gc08a3_3264x2448_24fps_addr_data,
		sizeof(gc08a3_3264x2448_24fps_addr_data)/sizeof(kal_uint16));
	pr_debug("[%s] custom1_End\n", __func__);
}

static void custom2_setting(void)
{
	pr_debug("[%s] custom2_Start, 1632x1224@30fps\n", __func__);
	table_write_cmos_sensor(gc08a3_1632x1224_addr_data,
		sizeof(gc08a3_1632x1224_addr_data)/sizeof(kal_uint16));
	pr_debug("[%s] custom1_End\n", __func__);
}
...
static BYTE gc08a3_bluey_common_data[CAMERA_EEPPROM_COMDATA_LENGTH] = { 0 };

static kal_uint16 read_gc08a3_bluey_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
...
static void read_gc08a3_bluey_module_data(UINT32 sensor_id)
{
// gc08a3_bluey_common_data
// length   : 64
// module&sensor&lens&vcm id : [0 ~ 3]
// QR       : [4 ~ 31]

	kal_uint16 idx = 0;
	kal_uint16 sn_length = 28;
	kal_uint32 sn_starAddr = 0x773;
	kal_uint32 moduleAddr = 0x02;
	kal_uint32 sensorAddr = 0x0B;
	kal_uint32 lensAddr = 0x0D;
	kal_uint32 vcmAddr = 0x0C;

	memset(gc08a3_bluey_common_data, 0,sizeof(gc08a3_bluey_common_data));
	// QR
	for(idx = 0; idx < sn_length; idx++)
	{
		gc08a3_bluey_common_data[4 + idx] = read_gc08a3_bluey_eeprom_module(sn_starAddr + idx);
	}
	//module id
	gc08a3_bluey_common_data[0] = read_gc08a3_bluey_eeprom_module(moduleAddr);
	//sensor id
	gc08a3_bluey_common_data[1] = read_gc08a3_bluey_eeprom_module(sensorAddr);
	//lens id
	gc08a3_bluey_common_data[2] = read_gc08a3_bluey_eeprom_module(lensAddr);
	//vcm id
	gc08a3_bluey_common_data[3] = read_gc08a3_bluey_eeprom_module(vcmAddr);
	for (idx = 0; idx < CAMERA_EEPPROM_COMDATA_LENGTH; idx = idx + 4)
	{
		pr_debug("cam data: %02x %02x %02x %02x\n",
		       gc08a3_bluey_common_data[idx],
		       gc08a3_bluey_common_data[idx + 1],
		       gc08a3_bluey_common_data[idx + 2],
		       gc08a3_bluey_common_data[idx + 3]);
	}
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				read_gc08a3_bluey_module_data(*sensor_id);
				return ERROR_NONE;
			}
				 ...
				 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_EEPROM_COMDATA:
		memcpy(feature_return_para_32, gc08a3_bluey_common_data,
...
UINT32 GC08A3_MIPI_RAW_BLUEY_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
