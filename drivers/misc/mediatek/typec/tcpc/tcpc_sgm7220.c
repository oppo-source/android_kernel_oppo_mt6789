...
static void sgm7220_process_tcpci_attach_state(struct sgm7220_chip *chip)
{
	...
	rv = tcpci_get_power_status(tcpc_dev);
	if (rv < 0)
		return;

	pr_info("%s vbus_level = %d\n", __func__, tcpc_dev->vbus_level);
 ...
 ****************************************************************/
int sgm7220_get_alert_status(struct tcpc_device *tcpc, uint32_t *alert, uint32_t *mask)
{
	struct sgm7220_chip *chip = tcpc_get_dev_data(tcpc);
	*alert = 0;
...
static struct tcpc_ops sgm7220_tcpc_ops = {
	...
	.get_alert_status_and_mask = sgm7220_get_alert_status,
...
static int sgm7220_tcpcdev_init(struct sgm7220_chip *chip, struct device *dev)
{
	...
	if (of_property_read_string(np, "sgm7220-tcpc,name", (char const **)&name) < 0)
		pr_err("%s, Failed to read sgm7220-tcpc,name\n", __func__);
	len = strlen(name);
