...
int wusb3801_get_alert_status(struct tcpc_device *tcpc, uint32_t *alert, uint32_t *mask)
{
	pr_info("%s enter \n", __func__);
	return 0;
}

static int wusb3801_get_power_status(struct tcpc_device *tcpc)
{
	pr_info("%s enter \n", __func__);
	return 0;
}
...
static struct tcpc_ops wusb3801_tcpc_ops = {
	...
	.get_alert_status_and_mask = wusb3801_get_alert_status,
...
static int wusb3801_tcpcdev_init(struct wusb3801_chip *chip, struct device *dev)
{
	...
	if (of_property_read_string(np, "wusb3801-tcpc,name", (char const **)&name) < 0)
		pr_err("%s, Failed to read wusb3801-tcpc,name\n", __func__);
	len = strlen(name);
	...
	chip->tcpc = tcpc_device_register(dev, desc, &wusb3801_tcpc_ops, chip);
