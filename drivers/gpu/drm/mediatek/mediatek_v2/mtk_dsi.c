...
bool panel_is_sonic_s(void)
{
	int prj_id = get_project();
	if (prj_id == 24695 || prj_id == 24694)
		return true;
	return false;
}
...
bool panel_is_fiji(void)
{
	int prj_id = get_project();
	if (24928 == prj_id || 24978 == prj_id || 24979 == prj_id)
		return true;
	return false;
}
...
/* #endif */ /* OPLUS_BUG_STABILITY*/
	...
	case DSI_VFP_IDLE_MODE:
	{
		...
		if (vfp_low_power) {
			...
			if (panel_is_aries() || panel_is_fiji())
...
/*endif*/
	...
	case GET_FRAME_HRT_BW_BY_DATARATE:
	{
		...
		*base_bw = mtk_dsi_get_frame_hrt_bw_base_by_datarate(crtc, dsi);
		...
		break;
//#ifdef OPLUS_BUG_STABILITY
/* MM.Display.LCD, 2021/12/27 add for cabc switch */
	case LCM_CABC:
	{
		struct mtk_dsi *dsi =
			container_of(comp, struct mtk_dsi, ddp_comp);
