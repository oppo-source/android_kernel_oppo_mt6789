...
bool g_silky_brightness_enable = true;
int mtk_drm_ioctl_get_silkybrightness_state(struct drm_device *dev, void *data,
	struct drm_file *file_priv)
{
	bool *silky_brightness_enable = (bool *)data;

	*silky_brightness_enable = g_silky_brightness_enable;
	return 0;
}

static void update_silkybrightness_state(void)
{
	struct mtk_ddp_comp *output_comp = NULL;
	unsigned int crtc0_connector_id = 0;
	struct drm_crtc *crtc = &default_comp->mtk_crtc->base;
	struct mtk_drm_private *private = crtc->dev->dev_private;
	struct mtk_ddp_comp *ccorr_comp = private->ddp_comp[DDP_COMPONENT_CCORR0];
	int ccorr_int_en = 1;
	...
	if (g_silky_brightness_enable && crtc0_connector_id == 35) {
		g_silky_brightness_enable = false;
		mtk_disp_ccorr_set_interrupt(ccorr_comp, &ccorr_int_en);
		DDPMSG("%s close silky brightness", __func__);
	} else if (!g_silky_brightness_enable && crtc0_connector_id == 32) {
		g_silky_brightness_enable = true;
		mtk_disp_ccorr_set_interrupt(ccorr_comp, &ccorr_int_en);
		DDPMSG("%s open silky brightness", __func__);
	}
}
							...
							enum mtk_ddp_io_cmd cmd, void *params)
{
	...
	if (comp->id == DDP_COMPONENT_CCORR0 && cmd == FRAME_DIRTY)
		update_silkybrightness_state();
